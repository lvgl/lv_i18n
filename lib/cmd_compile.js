...
module.exports.subparserArgsList = [
  ...
  },
  {
    ...
    options: {
      dest:     'output_raw',
      help:     'raw output file path for C code containing language variables',
      metavar:  '<path>'
    }
  },
  {
    args:     [ '--raw-header' ],
    options: {
      dest:     'output_raw_header',
      help:     'raw output file path for C header file containing index finding for optimize',
      metavar:  '<path>'
    }
  },
  {
...
`);
  ...
  let raw_idx = '#undef LV_I18N_OPTIMIZE\n';
  if (args.optimize) {
    raw_idx = getIDX(data);
  }
  ...
  if (args.output_raw_header) {
    writeFileSync(args.output_raw_header, raw_idx);
  }
  ...
  if (args.output) {
    ...
    txt_h = txt_h.replace(/\/\*IDX_START\*\/([\s\S]+)\/\*IDX_END\*\//,
      `${raw_idx}
////////////////////////////////////////////////////////////////////////////////`);
